...
\end{align}
$$
其中规定 $A=\{a_1,a_2,\dots,a_n\},B=\{b_1,b_2,\dots,b_n\}$。

我们观察到 $\prod\limits_{i=1}^{n}\sum\limits_{P:a_i\to b_{\sigma(i)}}\omega(P)$ 可知其实际上为所有从 $A$ 到 $B$ 的排列为 $\sigma$ 的路径组 $P$ 的 $\omega(P)$ 之和。
...
=&\sum_{P:A\to B}(-1)^{t(P)}\prod_{i=1}^n \omega(P_i)
\end{align}
$$
此处的 $P$ 为 $A$ 到 $B$ 的任意路径组。

我们设 $U$ 为不相交路径组，$V$ 为相交路径组。
...
\end{align}
$$
而对于任意一条相交路径，如 $P_i:x_1\to u \to y_1, P_j: x_2 \to u \to y_2$，我们有 $P_{i}':x_1 \to u \to y_2,P_{j}':x_2\to u \to y_1$，则有 $\omega(P)=\omega(P'),t(P)=t(P')\pm 1$。

因此我们对于任意相交路径组，都可以由此找到一一对应的相交路径组。

则我们可得 $\sum\limits_{V:A\to B}(-1)^{t(V)}\prod\limits_{i=1}^{n}\omega(V_i)=0$。

因此 $\det (M)=\sum\limits_{U:A\to B}(-1)^{t(U)}\prod\limits_{i=1}^n \omega(U_i)$。
